...
hw08_2020163053.js
- team : 12
- name : 전희망 (2020163053), 김휘영 (2023148033), 남의서 (2021147566)
----------------------------------------------------------------------------------*/
...
function setupKeyboardEvents() {
    document.addEventListener('keydown', (event) => {
        ...
        // toon levels에 대한 키보드 입력을 받기 위한 event 함수 정의
        else if (event.key >= '1' && event.key <= '5') {
            toonLevels = parseInt(event.key); //string으로 받아짐에 유의하기
...
async function initShader() {
    const vertexShaderSource = await readShaderFile('shVert_08.glsl');
    const fragmentShaderSource = await readShaderFile('shFrag_08.glsl');
...
function render() {
    ...
    shader.setInt("u_toonLevels", parseInt(toonLevels));
...
async function main() {
    try {
        ...
        shader.setVec3("material.diffuse", vec3.fromValues(0.8, 0.4, 0.1));
        shader.setVec3("material.specular", vec3.fromValues(0.8, 0.8, 0.8));
        shader.setFloat("material.shininess", shininess);
        shader.setVec3("u_viewPos", cameraPos);
        shader.setInt("u_toonLevels", parseInt(toonLevels)); // toonLevels는 1~5 사이 값으로 키보드로 부터 받아냄

        setupText(canvas, "TOON SHADING", 1);

        // 조건7. textOverlay는 2, 3번째 줄만 변경되도록 조정
